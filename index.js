...
let utils = {
  ...
  async getStreamInfo(streamerLogin, client) {
    let stream = await client.api(
      "GET",
      `${endpoints.STREAMS.GET}?user_login=${streamerLogin}`
    );
    if (stream.length !== 0) {
      stream = stream[0];
    }

    let user = await client.api(
      "GET",
      `${endpoints.USERS.GET.USER}?login=${streamerLogin}`
    );
    if (user.length === 0) {
    ...
    let channel = await client.api(
      "GET",
      `${endpoints.CHANNELS.GET_INFORMATION}?broadcaster_id=${user.id}`
    );
    if (channel.length === 0) {
...
class Client {
   ...
   */
  async api(method, endpoint, headers) {
    if (!endpoint) {
  ...
  async get({ endpoint, headers, params }) {
    if (!endpoint || typeof endpoint !== "string")
      throw new Error("Endpoint was not a string");
    const url = new URL(endpoint);
    if (!params || typeof params !== "object") params = {};
    const paramKeys = Object.keys(params);
    for (const i in paramKeys) {
      url.searchParams.set(
        encodeURIComponent(paramKeys[i]),
        encodeURIComponent(params[paramKeys[i]])
      );
    }
    return await this.api("get", url.href, headers);
  }
  ...
  async post({ endpoint, headers, params }) {
    ...
    return await this.api("post", url.href, headers);
  }
  ...
  async put({ endpoint, headers, params }) {
    ...
    return await this.api("put", url.href, headers);
  }
  ...
  async delete({ endpoint, headers, params }) {
    ...
    return await this.api("delete", url.href, headers);
  }

  getUserId(username) {
    return this.get({ endpoint: "/helix/users", params: { name: username } }); // endpoint.USERS.GET.USER
  }
  ...
  async setup() {
    ...
    if (token)
      out.clear().edit(chalk.greenBright("[1/4][1/3] ✅ Token fetched!")).end();
    else {
      ...
      if (secret && id && secret != "" && id != "") {
        out
          .clear()
          .edit(chalk.greenBright("[1/2][1/3][1/2] ✅ Secret and ID fetched!"))
          .end();
      } else {
        out
          .clear()
          .edit(chalk.redBright("[1/2][1/3][1/2] ❌ Secret and ID not found!"))
          .end();
        out
          .edit(
            chalk.gray(
      ...
      out
        .clear()
        .edit(chalk.greenBright("[1/4][1/3][2/2] ✅ Token fetched!"))
        .end();
    }
    ...
    if (res.status === 401) {
      ...
      out
        .clear()
        .edit(chalk.greenBright("[2/4][1/2][1/2] ✅ Token fetched!"))
        .end();
      ...
      if (res.data.status === 401) {
        out
          .clear()
          .edit(chalk.redBright("[2/4][1/2][2/2] ❌ Token invalid!"))
          .end();
        out.edit(chalk.gray("> Setting up config values...")).end();
        out.edit(chalk.gray("> Ending process...")).end();
        process.exit(1);
      ...
      out
        .clear()
        .edit(chalk.greenBright("[2/2][1/2][2/2] ✅ Token fetched!"))
        .end();
    ...
    out
      .edit(chalk.hex("#00ff00")("Setup completed - Welcome to EZ Twitch!"))
      .end();
